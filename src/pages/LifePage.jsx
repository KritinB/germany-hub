  ...
  return (
    <div style={{ background: '#07070D', minHeight: '100vh' }}>
      ...
      <div className="max-w-5xl mx-auto px-6 pt-28 pb-20">
        ...
        <Link to="/#life" className="inline-flex items-center gap-2 mb-8 text-sm font-medium" style={{ color: 'rgba(245,245,248,0.5)' }}>
              ...
              <div key={city.name} className="glass rounded-2xl p-6">
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs" style={{ color: 'rgba(245,245,248,0.4)' }}>{city.state}</div>
                ...
                <p className="text-xs mb-4 italic" style={{ color: 'rgba(245,245,248,0.5)' }}>{city.vibe}</p>
                <div className="grid grid-cols-2 gap-4 mb-3">
                  <div>
                    <div className="text-xs font-semibold mb-2" style={{ color: 'rgba(245,245,248,0.35)' }}>HIGHLIGHTS</div>
                    {city.pros.map((p, j) => (
                      <div key={j} className="flex items-center gap-1.5 text-xs mb-1" style={{ color: 'rgba(245,245,248,0.6)' }}>
                        <div className="w-1 h-1 rounded-full flex-shrink-0" style={{ background: '#34D399' }} />{p}
                      </div>
                    ))}
                  </div>
                  <div>
                    <div className="text-xs font-semibold mb-2" style={{ color: 'rgba(245,245,248,0.35)' }}>DOWNSIDES</div>
                    {city.cons.map((c, j) => (
                      <div key={j} className="flex items-center gap-1.5 text-xs mb-1" style={{ color: 'rgba(245,245,248,0.4)' }}>
                ...
                <div className="pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                  <span className="text-xs" style={{ color: 'rgba(245,245,248,0.35)' }}>Avg 1BR Rent: </span>
          ...
          <div className="grid md:grid-cols-2 gap-5">
            {moreCities.map((city, i) => (
              <div key={city.name} className="glass rounded-2xl p-6">
                ...
                <p className="text-xs mb-3 italic" style={{ color: 'rgba(245,245,248,0.5)' }}>{city.vibe}</p>
                <div className="space-y-1 mb-3">
                  {city.pros.slice(0, 3).map((p, j) => (
                    <div key={j} className="flex items-center gap-1.5 text-xs" style={{ color: 'rgba(245,245,248,0.6)' }}>
                      <div className="w-1 h-1 rounded-full flex-shrink-0" style={{ background: '#34D399' }} />{p}
                    </div>
                  ))}
                </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-10">
          ...
          <div className="mt-4 glass rounded-2xl p-5">
            <h4 className="font-bold text-white mb-3">Best Housing Search Websites</h4>
            <div className="grid md:grid-cols-3 gap-3">
              {[
                ...
                <div key={i} className="rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  <div className="text-xs" style={{ color: 'rgba(245,245,248,0.45)' }}>{s.desc}</div>
                </div>
              ))}
            </div>
          </div>
            ...
            {tips.map((tip, i) => {
              ...
              return (
                <div key={tip.title} className="glass rounded-2xl p-6">
                  ...
                  <p className="text-sm leading-relaxed" style={{ color: 'rgba(245,245,248,0.5)' }}>{tip.desc}</p>
                </div>
              )
            })}
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="glass rounded-2xl p-8 mb-10">
          <div className="flex items-start gap-4 mb-6">
            ...
            <div>
              <h2 className="text-xl font-bold text-white mb-1">Indian Community by City</h2>
              <p className="text-sm" style={{ color: 'rgba(245,245,248,0.5)' }}>40,000+ Indian students and 200,000+ Indian expats make Germany very welcoming for Indians</p>
            </div>
          </div>
            ...
            {indianCommunity.map((item, i) => (
              <div key={i} className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                <div className="flex items-center gap-2 mb-2">
                  <MapPin size={12} color="#C9A46D" />
                  <span className="font-semibold text-white text-sm">{item.city}</span>
                </div>
                <div className="grid md:grid-cols-2 gap-2 text-xs" style={{ color: 'rgba(245,245,248,0.5)' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="rounded-2xl p-8 text-center" style={{ background: 'linear-gradient(135deg, rgba(201,164,109,0.08), rgba(91,140,255,0.05))', border: '1px solid rgba(201,164,109,0.15)' }}>
          <h3 className="text-xl font-bold text-white mb-2">Now look at scholarships to fund your studies</h3>
          <p className="text-sm mb-6" style={{ color: 'rgba(245,245,248,0.5)' }}>DAAD and foundation scholarships can cover your living expenses entirely.</p>
